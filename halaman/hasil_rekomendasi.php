...
while ($row = $result->fetch_assoc()) {
    ...
    $antutuSql = "
        ...
        WHERE sp.`Nama Produk` = '" . $conn->real_escape_string($row['Nama Produk']) . "'
    ";
...
echo '<br>';
echo '<h1 class="text-center mt-5"><a href="rekomendasi.php" class="btn btn-dark back-button" style="  margin-right: 20px; margin-top: -5px">&lt;</a>
 Top 10 Rekomendasi Smartphone</h1>';
...
foreach ($smartphones as $smartphone) {
    ...
    echo "<p class='card-text' style='color: blue; font-weight: bold; font-size: 0.8em;'>Skor  " . ucwords(str_replace('_', ' ', $kebutuhan)) . ": " . number_format($kebutuhanScore, 2) . "</p>";
    ...
    echo "<a href='#' class='btn btn-primary mx-1' target='_blank'>Penilaian</a>";
    echo "</div>";
    echo "</div>";
    echo "</div>";
    ...
    $totalGamingScore = 0;
    foreach ($gamingSpecs as $spec) {
        echo "<tr><td>" . htmlspecialchars($spec[0]) . "</td><td>" . htmlspecialchars($spec[1]) . "</td><td>" . htmlspecialchars($spec[2]) . "</td><td>" . number_format($gamingBobot, 2) . "</td></tr>";
        $totalGamingScore += $spec[2] * $gamingBobot;
    }
    echo "</tbody></table>";
    echo "<p>Wj = 1 / banyak spesifikasi = 1 / " . count($gamingSpecs) . " = " . number_format($gamingBobot, 2) . "</p>";
    echo "<p>Perhitungan Kriteria: ";
    foreach ($gamingSpecs as $index => $spec) {
        echo "(" . number_format($gamingBobot, 2) . " × " . htmlspecialchars($spec[2]) . ") = " . number_format($gamingBobot * $spec[2], 2);
        if ($index < count($gamingSpecs) - 1) {
            echo " + ";
        }
    }
    echo " = " . number_format($totalGamingScore, 2) . "</p>";
    echo "<p>Skor Kriteria: " . number_format($totalGamingScore, 2) . "</p>";
    echo "</div>";
    ...
    foreach ($fotografiSpecs as $spec) {
        echo "<tr><td>" . htmlspecialchars($spec[0]) . "</td><td>" . htmlspecialchars($spec[1]) . "</td><td>" . htmlspecialchars($spec[2]) . "</td><td>" . number_format($fotografiBobot, 2) . "</td></tr>";
        $totalFotografiScore += $spec[2] * $fotografiBobot;
    }
    echo "</tbody></table>";
    echo "<p>Wj = 1 / banyak spesifikasi = 1 / " . count($fotografiSpecs) . " = " . number_format($fotografiBobot, 2) . "</p>";
    echo "<p>Perhitungan Kriteria: ";
    foreach ($fotografiSpecs as $index => $spec) {
        echo "(" . number_format($fotografiBobot, 2) . " × " . htmlspecialchars($spec[2]) . ") = " . number_format($fotografiBobot * $spec[2], 2);
        if ($index < count($fotografiSpecs) - 1) {
            echo " + ";
        }
    }
    echo " = " . number_format($totalFotografiScore, 2) . "</p>";
    echo "<p>Skor Kriteria: " . number_format($totalFotografiScore, 2) . "</p>";
    echo "</div>";
    ...
    foreach ($kontenKreatorSpecs as $spec) {
        echo "<tr><td>" . htmlspecialchars($spec[0]) . "</td><td>" . htmlspecialchars($spec[1]) . "</td><td>" . htmlspecialchars($spec[2]) . "</td><td>" . number_format($kontenKreatorBobot, 2) . "</td></tr>";
        $totalKontenKreatorScore += $spec[2] * $kontenKreatorBobot;
    }
    echo "</tbody></table>";
    echo "<p>Wj = 1 / banyak spesifikasi = 1 / " . count($kontenKreatorSpecs) . " = " . number_format($kontenKreatorBobot, 2) . "</p>";
    echo "<p>Perhitungan Kriteria: ";
    foreach ($kontenKreatorSpecs as $index => $spec) {
        echo "(" . number_format($kontenKreatorBobot, 2) . " × " . htmlspecialchars($spec[2]) . ") = " . number_format($kontenKreatorBobot * $spec[2], 2);
        if ($index < count($kontenKreatorSpecs) - 1) {
            echo " + ";
        }
    }
    echo " = " . number_format($totalKontenKreatorScore, 2) . "</p>";
    echo "<p>Skor Kriteria: " . number_format($totalKontenKreatorScore, 2) . "</p>";
    echo "</div>";
    ...
    foreach ($sehariHariSpecs as $spec) {
        echo "<tr><td>" . htmlspecialchars($spec[0]) . "</td><td>" . htmlspecialchars($spec[1]) . "</td><td>" . htmlspecialchars($spec[2]) . "</td><td>" . number_format($sehariHariBobot, 2) . "</td></tr>";
        $totalSehariHariScore += $spec[2] * $sehariHariBobot;
    }
    echo "</tbody></table>";
    echo "<p>Wj = 1 / banyak spesifikasi = 1 / " . count($sehariHariSpecs) . " = " . number_format($sehariHariBobot, 2) . "</p>";
    echo "<p>Perhitungan Kriteria: ";
    foreach ($sehariHariSpecs as $index => $spec) {
        echo "(" . number_format($sehariHariBobot, 2) . " × " . htmlspecialchars($spec[2]) . ") = " . number_format($sehariHariBobot * $spec[2], 2);
        if ($index < count($sehariHariSpecs) - 1) {
            echo " + ";
        }
    }
    echo " = " . number_format($totalSehariHariScore, 2) . "</p>";
    echo "<p>Skor Kriteria: " . number_format($totalSehariHariScore, 2) . "</p>";
    echo "</div>";
    echo "</div>";
    echo "</div>";
    echo "<div class='ml-3'>";
